...
print("==========================================================================")
print("🚀 [UPDATING EXPENSE MODULE] APPLYING 4 SMART ENHANCEMENTS...")
print("==========================================================================")

# 1. Update src/parsers/expenseParser.ts with Banswara 372 KM Rule
parser_code = """import * as XLSX from 'xlsx';
...
with open('src/parsers/expenseParser.ts', 'w', encoding='utf-8') as f:
    f.write(parser_code)
print("✅ 1. src/parsers/expenseParser.ts updated with Banswara 372 KM rule.")

# 2. Update src/components/ExpenseWorkspace.tsx with:
# - Freeze pane up to Actual Station
# - Smart Other Expenses Modal (Parking, Room, Ticket, etc. -> Other Exp + Remark)
# - Banswara 372 KM Rule
# - Head/Type/Amount Box with "Hide Value" Toggle
ui_code = """import React, { useState, useMemo } from 'react';
import { 
...
export const ExpenseWorkspace: React.FC<Props> = ({ onBack }) => {
  ...
  // 🌟 REQUIREMENT 4: HIDE VALUE TOGGLE (HEAD | TYPE | AMOUNT BOX)
  ...
  // 🌟 REQUIREMENT 4: COMPLETE HEAD | TYPE | AMOUNT BOX
  ...
  // 🌟 REQUIREMENT 2: SMART OTHER EXPENSES MODAL STATE
  ...
  const [smartItems, setSmartItems] = useState<SmartExpenseItem[]>([
    { id: '1', label: 'Parking bill', amount: 10 },
    { id: '2', label: 'Bus/Train Ticket', amount: 500 },
    { id: '3', label: 'Room / Stay', amount: 600 }
  ]);
  const [newSmartLabel, setNewSmartLabel] = useState('');
  const [newSmartAmount, setNewSmartAmount] = useState('');
  ...
  const handleCellChange = (index: number, field: keyof ExpenseDayRow, val: any) => {
    ...
    // 🌟 REQUIREMENT 3: BANSWARA ALWAYS 372 KM
  ...
  // 🌟 REQUIREMENT 2: APPLY SMART OTHER EXPENSES (SUM TO OTHER EXPENSE + REMARK NOTE)
  const handleOpenSmartExpense = (rowIdx: number) => {
    const row = rows[rowIdx];
    setActiveSmartRowIdx(rowIdx);

    // If row already has other expense, pre-seed
    const currentVal = parseFloat(String(row.otherExpense || 0)) || 0;
    if (currentVal > 0 && smartItems.length === 0) {
      setSmartItems([{ id: 'init_1', label: 'Other Exp', amount: currentVal }]);
    }
  };

  const handleAddSmartItem = () => {
    const amt = parseFloat(newSmartAmount) || 0;
    if (!newSmartLabel.trim() || amt <= 0) return;

    setSmartItems(prev => [
      ...prev,
      { id: 'smart_' + Date.now(), label: newSmartLabel.trim(), amount: amt }
    ]);
    setNewSmartLabel('');
    setNewSmartAmount('');
  };

  const handleQuickAddPreset = (label: string, defaultAmt: number) => {
    setSmartItems(prev => [
      ...prev,
      { id: 'preset_' + Date.now(), label, amount: defaultAmt }
    ]);
  };
  ...
  const handleApplySmartExpense = () => {
    ...
    // Total of all items
    const sumOther = smartItems.reduce((acc, it) => acc + (it.amount || 0), 0);
    targetRow.otherExpense = sumOther > 0 ? sumOther : 0;

    // Build remark note: e.g. "Parking: 10, Ticket: 500, Room: 600"
    const breakdownText = smartItems
      .filter(it => it.amount > 0)
      .map(it => `${it.label}: ${it.amount}`)
      .join(', ');

    if (breakdownText) {
      const existingRemark = String(targetRow.remark || '').trim();
      if (existingRemark && existingRemark !== 'ok') {
        targetRow.remark = `${existingRemark}; ${breakdownText}`;
      } else {
    ...
    setStatusMsg(`🎉 Smart Other Expense applied: ₹${sumOther} added to Day #${targetRow.srNo} & Remark updated!`);
  ...
  const totals = useMemo(() => {
    ...
    // 🌟 If hideMiscValues is true, misc becomes 0
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      // 🌟 Check if imported file had 0 misc (like July) -> auto set hideMiscValues
      ...
      setStatusMsg(`🎉 SUCCESS! '${file.name}' imported: ${parsed.monthCode} (${parsed.rows.length} Days) loaded with Banswara 372 KM rule!`);
    } catch (err: any) {
  ...
  // 🌟 REQUIREMENT 4: EXPORT OFFICIAL CSV WITH EXACT HIDE VALUE SUPPORT
  const handleExportCSV = () => {
    ...
    // 🌟 REQUIREMENT 4: IF HIDE VALUE IS ON, SEND BLANK (JUST LIKE JULY CSV!)
    if (!hideMiscValues) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] bg-emerald-950 text-emerald-300 border border-emerald-500/40 px-3 py-1 rounded-full font-mono font-bold flex items-center gap-1.5 shadow-lg shadow-emerald-950/50">
            <Sparkles size={13} className="text-emerald-400 animate-pulse" /> FREEZE PANE &bull; BANSWARA 372 KM &bull; SMART OTHER EXPENSES
          </span>
        </div>
      </div>
      ...
      {/* 4. PERFORMANCE ANALYTICAL METRICS BOX */}
      {performanceMetrics && (
        <div className="p-3.5 bg-slate-900/90 rounded-2xl border border-purple-500/40 shadow-inner flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <div className="text-[10px] text-slate-400">Master List: {performanceMetrics.totalDr} Drs &bull; Field Days: {performanceMetrics.workingDays}</div>
            </div>
          </div>
      ...
      {/* 🌟 5. MAIN 19-COLUMN DAY-WISE EXPENSE TABLE (WITH PANEL FREEZE UP TO ACTUAL STATION!) */}
          ...
          <table className="w-full text-left text-xs border-separate border-spacing-0">
            ...
            <thead className="sticky top-0 z-30 bg-slate-950">
              <tr>
                ...
                {/* ❄️ FROZEN 3: ACTUAL STATION (DIVIDER WITH GLOWING SHADOW) */}
                ...
                rows.map((row, idx) => {
                  ...
                  return (
                    <tr key={idx} className={`transition group ${
                      ...
                      {/* ❄️ FROZEN 3: ACTUAL STATION (WITH DIVIDER SHADOW) */}
                      ...
                      {/* 🌟 REQUIREMENT 2: SMART OTHER EXPENSE TRIGGER BUTTON */}
                      <td className="p-1 border-b border-r border-slate-800/60">
                        <div className="flex items-center gap-1">
                          <input
                            type="text"
                          ...
                          <button
                            ...
                            className="p-1 text-yellow-400 hover:text-white bg-yellow-950/60 hover:bg-yellow-900 border border-yellow-500/30 rounded transition cursor-pointer"
                            title="Open Smart Breakdown (Parking + Ticket + Room)"
                          >
                            <Calculator size={11} />
                          </button>
                        </div>
      ...
      {/* 🌟 6. ALLOWANCE CALCULATION BOX & COMPLETE HEAD/TYPE/AMOUNT (WITH HIDE VALUE TOGGLE!) */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-xl space-y-3">
          ...
          <div className="flex items-center justify-between pb-2 border-b border-slate-800">
            ...
            {/* 🌟 REQUIREMENT 4: HIDE VALUE TOGGLE */}
            <button
              type="button"
              onClick={() => {
      ...
      {/* 🌟 REQUIREMENT 2: SMART OTHER EXPENSE MODAL */}
      {activeSmartRowIdx !== null && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border-2 border-yellow-500/60 rounded-3xl max-w-lg w-full p-5 shadow-2xl space-y-4">
            ...
            <div className="space-y-1.5">
              <span className="text-[10px] text-slate-400 font-bold uppercase">Quick Add Presets:</span>
              <div className="flex flex-wrap items-center gap-1.5">
                <button
                  type="button"
                  onClick={() => handleQuickAddPreset('Parking bill', 10)}
                  className="px-2.5 py-1 bg-slate-950 hover:bg-slate-800 text-yellow-300 border border-yellow-500/30 rounded-lg text-xs font-semibold cursor-pointer"
                >
                  + Parking (₹10)
                </button>
                <button
                  type="button"
                  onClick={() => handleQuickAddPreset('Ticket', 500)}
                  className="px-2.5 py-1 bg-slate-950 hover:bg-slate-800 text-yellow-300 border border-yellow-500/30 rounded-lg text-xs font-semibold cursor-pointer"
                >
                  + Ticket (₹500)
                </button>
                <button
                  type="button"
                  onClick={() => handleQuickAddPreset('Room / Hotel', 600)}
                  className="px-2.5 py-1 bg-slate-950 hover:bg-slate-800 text-yellow-300 border border-yellow-500/30 rounded-lg text-xs font-semibold cursor-pointer"
                >
                  + Room (₹600)
                </button>
                <button
                  type="button"
                  onClick={() => handleQuickAddPreset('Toll Tax', 120)}
                  className="px-2.5 py-1 bg-slate-950 hover:bg-slate-800 text-yellow-300 border border-yellow-500/30 rounded-lg text-xs font-semibold cursor-pointer"
                >
                  + Toll (₹120)
                </button>
            ...
            <div className="p-3 bg-slate-950 rounded-xl border border-slate-800 space-y-2 text-xs">
              <span className="text-[10px] text-yellow-400 font-bold uppercase">Custom Item:</span>
              <div className="flex items-center gap-2">
                <input
                  type="text"
                  placeholder="Expense Item (e.g. Fuel, Auto)..."
                  value={newSmartLabel}
                  onChange={e => setNewSmartLabel(e.target.value)}
                  className="flex-1 bg-slate-900 border border-slate-700 text-white rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-yellow-400"
                />
                <input
                  type="number"
                  placeholder="₹ Amount"
                  value={newSmartAmount}
                  onChange={e => setNewSmartAmount(e.target.value)}
                  className="w-24 bg-slate-900 border border-slate-700 text-yellow-300 font-mono font-bold text-right rounded-lg px-2.5 py-1.5 focus:outline-none focus:border-yellow-400"
                />
                <button
                  type="button"
                  onClick={handleAddSmartItem}
                  className="px-3 py-1.5 bg-yellow-500 hover:bg-yellow-400 text-slate-950 font-bold rounded-lg cursor-pointer"
                >
                  + Add
                </button>
              </div>
            ...
            {/* Added Items List */}
            <div className="max-h-40 overflow-y-auto space-y-1.5 border border-slate-800 rounded-xl p-2 bg-slate-950/60">
              {smartItems.length === 0 ? (
                <div className="py-4 text-center text-xs text-slate-500 font-mono">No items added yet.</div>
              ) : (
                smartItems.map(it => (
                  <div key={it.id} className="flex items-center justify-between p-2 rounded-lg bg-slate-900 border border-slate-800 text-xs">
                    <span className="font-semibold text-white">{it.label}</span>
                    <div className="flex items-center gap-3">
                      <span className="font-mono font-bold text-yellow-300">₹{it.amount}</span>
                      <button
                        type="button"
            ...
            <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
              <div>
                ...
                <div className="text-base font-black text-yellow-300 font-mono">
                  ₹{smartItems.reduce((acc, it) => acc + (it.amount || 0), 0)}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 text-slate-950 font-bold text-xs rounded-xl shadow cursor-pointer"
                >
                  Apply to Day &amp; Update Remark
                </button>
              </div>
            </div>
...
with open('src/components/ExpenseWorkspace.tsx', 'w', encoding='utf-8') as f:
    f.write(ui_code)
print("✅ 2. src/components/ExpenseWorkspace.tsx updated with 4 smart features.")
