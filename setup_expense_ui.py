# Create ExpenseWorkspace.tsx
expense_ui_code = """import React, { useState, useEffect, useMemo } from 'react';
import { 
...
with open('src/components/ExpenseWorkspace.tsx', 'w', encoding='utf-8') as f:
    f.write(expense_ui_code)
print("✅ src/components/ExpenseWorkspace.tsx created.")

# Update WebDataWorkspace.tsx to connect ExpenseWorkspace
with open('src/components/WebDataWorkspace.tsx', 'r', encoding='utf-8') as f:
    web_data_code = f.read()

if "import { ExpenseWorkspace } from './ExpenseWorkspace';" not in web_data_code:
    web_data_code = web_data_code.replace(
        "import { IncentiveWorkspace } from './IncentiveWorkspace';",
        "import { IncentiveWorkspace } from './IncentiveWorkspace';\nimport { ExpenseWorkspace } from './ExpenseWorkspace';"
    )

# Replace Coming soon button with active button
web_data_code = web_data_code.replace("type ViewState = 'web-data' | 'statement' | 'stockwise-statement' | 'earn' | 'incentive';", "type ViewState = 'web-data' | 'statement' | 'stockwise-statement' | 'earn' | 'incentive' | 'expense';")

old_expense_card = """            {/* OPTION 2: EXPENSE (Placeholder / Coming Soon) */}
...
new_expense_card = """            {/* OPTION 2: 🌟 ACTIVE EXPENSE WORKSPACE */}
...
if old_expense_card in web_data_code:
    web_data_code = web_data_code.replace(old_expense_card, new_expense_card)

# Add Render ExpenseWorkspace view
old_render_end = """      {/* 3️⃣ LEVEL 3 (B): 🌟 INCENTIVE WORKSPACE */}
      {currentView === 'incentive' && (
        <IncentiveWorkspace onBack={() => setCurrentView('earn')} />
      )}"""

new_render_end = """      {/* 3️⃣ LEVEL 3 (B): 🌟 INCENTIVE WORKSPACE */}
      {currentView === 'incentive' && (
        <IncentiveWorkspace onBack={() => setCurrentView('earn')} />
      )}
...
if old_render_end in web_data_code:
    web_data_code = web_data_code.replace(old_render_end, new_render_end)

with open('src/components/WebDataWorkspace.tsx', 'w', encoding='utf-8') as f:
    f.write(web_data_code)
print("✅ src/components/WebDataWorkspace.tsx updated with active Expense Workspace.")
