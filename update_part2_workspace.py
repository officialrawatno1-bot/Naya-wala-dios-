...
print("==========================================================================")
print("🚀 [PART 2/3] UPDATING UI: DUAL-HAND CLOCK & NOT-AVAILABLE DAYS ENGINE...")
print("==========================================================================")
...
export const DailyWorkingWorkspace: React.FC<Props> = ({ onBack }) => {
  ...
  // 🌟 DUAL-HAND INTERACTIVE CLOCK STATE
  ...
  const [clockNotAvailableDays, setClockNotAvailableDays] = useState<string[]>([]);
  ...
  const handleOpenClockModal = (doc: DoctorFieldProfile) => {
    ...
    setClockNotAvailableDays(doc.notAvailableDays || []);
    setClockArea(doc.area || 'Hospital Road');
    setClockNotes(doc.notes || '');
    setClockMode('HOUR');
  };

  // 🌟 DUAL-HAND ROTATION (Hour vs Minute depending on mode)
  ...
  const handleSaveClockSchedule = () => {
    ...
    const available = allDays.filter(d => !clockNotAvailableDays.includes(d));
    ...
    const updatedProfile: DoctorFieldProfile = {
      ...
      period: clockPeriod,
      area: clockArea,
      notAvailableDays: clockNotAvailableDays,
      availableDays: available,
      hasSpecificDays: clockNotAvailableDays.length > 0,
      notes: clockNotes
    };
      ...
      const updatedCalls = currentPlan.plannedCalls.map(p => {
        if (p.srNo === clockTargetDoc.srNo) {
          return {
            ...
            otRemarks: clockNotAvailableDays.length > 0 ? `Off on: ${clockNotAvailableDays.join(', ')}` : (clockNotes || 'Available')
          };
        }
        return p;
      });
    ...
    setStatusMsg(`🎉 Dr. ${clockTargetDoc.doctorName} schedule updated to ${formattedTime} at ${clockArea}!`);
    setTimeout(() => setStatusMsg(null), 3000);
  };

  // Generate automated plan with strict Day & Not-Available day validation
  const generatePlanForAreas = (areasChoice: string[], dateStr: string) => {
    ...
    if (activeExStation) {
      ...
      matchedDoctors = allProfilesList.filter(d => {
        ...
        // Check if doctor is explicitly off on this day
        if (d.notAvailableDays && d.notAvailableDays.includes(dayShort)) {
          return false;
        }
    ...
    const plannedItems: PlannedCallItem[] = matchedDoctors.map(doc => {
      ...
      const isOffToday = doc.notAvailableDays && doc.notAvailableDays.includes(dayShort);
      const remark = isOffToday ? `🚫 Off on ${dayShort}` : (doc.notes || 'Available');
      ...
      return {
        ...
        recencyTier: tier,
        status: isOffToday ? 'IN_OT' : 'PLANNED',
        notes: ''
      };
    });
  ...
  // Remaining doctors in area (No cap hiding doctors)
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 md:p-6 shadow-xl space-y-5 max-w-7xl mx-auto text-slate-100">
          ...
          <div>
            <h1 className="text-base md:text-lg font-bold text-white flex items-center gap-2">
              ...
              <span className="text-[10px] bg-purple-500/20 text-purple-300 border border-purple-500/40 px-2.5 py-0.5 rounded-full font-mono font-bold flex items-center gap-1">
                <Sparkles size={11} className="text-purple-400" /> Dual-Hand Clock &bull; Off-Days Protected
              </span>
            </h1>
            <p className="text-xs text-slate-400">BE: BANWARI LAL MEENA &bull; HQ: UDAIPUR &bull; Hour + Minute Clock &bull; Color-Coded Recency</p>
          </div>
        </div>
      ...
      <CloudSyncBar
        storageKey="field/daily_working_system_v4"
        ...
        onLoadData={(cloudData: any) => {
          ...
          if (cloudData.dayPlans) {
            ...
            try { localStorage.setItem('dios_daily_plans_v4', JSON.stringify(cloudData.dayPlans)); } catch (e) {}
          }
        }}
        onSaveLocal={() => {
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-400">Select multiple hospitals at once &bull; Doctor sitting days are auto-checked!</p>
                </div>
              </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead className="sticky top-0 bg-slate-950 text-slate-400 font-bold uppercase border-b border-slate-800 z-10">
                  <tr>
                    ...
                    <th className="p-2.5 min-w-[150px] text-amber-400">Sitting / Notes</th>
        ...
        <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 shadow-xl space-y-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-800">
            <div>
              ...
              <p className="text-xs text-slate-400">Tap Clock ⏰ to turn dial &bull; Set Hospital &bull; Choose Sitting Days</p>
            </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead className="sticky top-0 bg-slate-950 text-slate-400 font-bold uppercase border-b border-slate-800">
                <tr>
                  ...
                  <th className="p-2.5 min-w-[150px] text-rose-400">🚫 Not Available Days</th>
                  <th className="p-2.5 min-w-[140px] text-slate-400">Notes / Timing</th>
                ...
                {Object.values(profiles)
                  ...
                  .map(doc => (
                    <tr key={doc.srNo} className="hover:bg-slate-800/40 transition">
                      ...
                      {/* Not Available Days */}
                      <td className="p-2 font-sans text-rose-400 text-[10px]">
                        {doc.notAvailableDays.length > 0 ? doc.notAvailableDays.join(', ') : 'All Days (Mon-Sat)'}
                      </td>
      ...
      {/* ========================================================================= */}
      {/* 5. 🌟 SCREENSHOT-MATCHING DUAL-HAND TOUCH CLOCK & OFF-DAYS MODAL          */}
      {/* ========================================================================= */}
      {clockTargetDoc && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-3 md:p-6 animate-in fade-in duration-200">
          <div className="bg-white text-slate-900 rounded-3xl max-w-sm w-full overflow-hidden shadow-2xl flex flex-col p-5 space-y-3">
            ...
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-black text-slate-800 tracking-wide">
                  Set Time &bull; Dr. {clockTargetDoc.doctorName.split(' ')[0]}
                </h3>
              </div>
              <button onClick={() => setClockTargetDoc(null)} className="text-slate-400 hover:text-slate-600"><X size={18} /></button>
            </div>

            {/* Mode Switcher: Hour Hand vs Minute Hand (Chhota Kanta) */}
            ...
            {/* 🌟 ANALOG TOUCH DIAL WITH DUAL HANDS (HOUR + MINUTE HANDS) */}
            <div className="flex flex-col items-center justify-center select-none">
              <div className="relative w-60 h-60 flex items-center justify-center">
                ...
                <svg
                  ref={clockDialRef}
                  viewBox="0 0 260 260"
                  className="w-60 h-60 cursor-pointer touch-none"
                ...
                >
                  ...
                  {/* 🌟 2. MINUTE HAND (Chhota / Barik Kanta) */}
                  <line
                    x1="130"
                    y1="130"
              ...
              <div className="flex items-center gap-1.5 mt-2">
                <span className="text-[10px] text-slate-400 font-bold uppercase">Quick Min:</span>
                {QUICK_MINUTES.map(m => (
                  <button
                    key={m}
                    type="button"
            ...
            <div className="space-y-2.5 pt-2 border-t border-slate-100 text-xs">
              ...
              <div>
                ...
                <div className="flex justify-between items-center bg-slate-50 p-2 rounded-xl border border-slate-200">
                  {['MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'].map(dayKey => {
                    const isOff = clockNotAvailableDays.includes(dayKey);
                    return (
                      <button
                        key={dayKey}
                        type="button"
                        onClick={() => {
                          if (clockNotAvailableDays.includes(dayKey)) {
                            setClockNotAvailableDays(clockNotAvailableDays.filter(d => d !== dayKey));
                          } else {
                            setClockNotAvailableDays([...clockNotAvailableDays, dayKey]);
                          }
                        }}
                        ...
                        title={isOff ? "Doctor is OFF on this day" : "Doctor is AVAILABLE"}
                      >
                        {dayKey}
                      </button>
                    );
                ...
                <div className="text-[9px] text-slate-400 mt-1 flex justify-between">
                  ...
                  <span>🔴 Red = Doctor Off / No Visit</span>
                </div>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-xs font-bold rounded-xl px-3 py-1.5 focus:outline-none focus:border-[#8B5CF6] cursor-pointer"
                >
              ...
              <div>
                <input
                  ...
                  placeholder="e.g. Thu & Fri 1:00 - 3:30 PM"
                  className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-xs rounded-xl px-3 py-1.5"
                />
              </div>
...
print("✅ Part 2 complete! Dual-hand clock & Off-days UI updated.")
