...
print("==========================================================================")
print("🧠 [2000 IQ FIX] BLOCKING DASHED SEPARATORS FROM OVERWRITING CHEMIST NAME...")
print("==========================================================================")
...
export async function parseModiRetailerPdf(file: File): Promise<{ records: RetailerSaleRecord[]; detectedMonthCode?: string }> {
  ...
  for (const line of virtualLines) {
    ...
    if (!dataRow) {
      ...
      // 🛑 CRITICAL CHECK 1: Skip dashes, dividers, lines starting with symbols
      if (
        !trimmed ||
        trimmed.startsWith('-') ||
        trimmed.startsWith('=') ||
      ...
      // 🛑 CRITICAL CHECK 2: If it's a product line (e.g. 0 qty line), DO NOT change currentParty!
      if (prod) {
        continue;
      }

      // 🛑 CRITICAL CHECK 3: Must not be any report title or column header
...
print("✅ 1. modiRetailerParser.ts updated with divider & 0-qty product protection.")
...
print("\n🎉 ALL DONE! Modi July 100% Exact Matching Deployed!")
