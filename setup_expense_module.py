...
print("==========================================================================")
print("🚀 [BUILDING EXPENSE STATEMENT MODULE] INTEGRATING BACKEND & FRONTEND...")
print("==========================================================================")

# 1. Create expense_engine.py
...
print("✅ expense_engine.py created.")

# 2. Update server.py to add /api/fetch-expense endpoint
with open('server.py', 'r', encoding='utf-8') as f:
    server_code = f.read()

if 'from expense_engine import fetch_cbo_expense' not in server_code:
...
if '@app.post("/api/fetch-expense")' not in server_code:
    endpoint_code = """
@app.post("/api/fetch-expense")
def api_fetch_expense(req: MonthRequest):
    return fetch_cbo_expense(req.from_month)
"""
    server_code += endpoint_code

with open('server.py', 'w', encoding='utf-8') as f:
    f.write(server_code)
print("✅ server.py updated with /api/fetch-expense endpoint.")

# 3. Create functions/api/fetch-expense.ts
cf_expense_proxy = """export async function onRequest(context: any) {
  ...
  try {
    ...
    const apiRes = await fetch("/api/fetch-expense", {
      method: "POST",
...
"""
with open('functions/api/fetch-expense.ts', 'w', encoding='utf-8') as f:
    f.write(cf_expense_proxy)
print("✅ functions/api/fetch-expense.ts created.")
