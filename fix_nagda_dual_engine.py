...
print("==========================================================================")
print("🧠 [1000 IQ] UPGRADING NAGDA PARSER TO DUAL-INVARIANT ENGINE...")
print("==========================================================================")

new_parser_code = '''import * as pdfjsLib from 'pdfjs-dist';
...
const LOCATION_KEYWORDS = [
  ...
  'FATHAPURA', 'P.W', 'J.K PARAS', 'TOKOR CHORAYA', 'D.G', 'AMBAMATA',
  'MADHUBAN', 'SURAJPOL', 'SEVASHRAM', 'CHITRKUT NAG'
];
...
export async function parseNagdaRetailerPdf(file: File): Promise<{ 
  ...
  let reportFormat: 'ITEM_WISE' | 'PARTY_WISE' = 'PARTY_WISE';
  ...
  for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
    ...
    for (const lineItems of lineBins) {
      ...
      if (fullLine) {
        ...
        // Detect Format: ITEM/ITEM WISE vs PARTY/ITEM WISE
        ...
        if (!detectedMonthCode) {
          const rangeMatch = upper.match(/FROM\\s+(\\d{2})[-/](\\d{2})[-/](\\d{4})[-–—TO\\s]+(\\d{2})[-/](\\d{2})[-/](\\d{4})/);
          if (rangeMatch) {
            ...
            const singleMatch = upper.match(/FROM\\s+\\d{2}[-/](\\d{2})[-/](\\d{4})/);
  ...
  for (const line of virtualLines) {
    ...
    if (!dataRow) {
      ...
      if (
        ...
        upper.includes('ITEMWISE') || upper.includes('PARTYWISE') ||
        upper.includes('REPORTFOR') || upper.includes('DIOSLIFESCINCES') || 
        upper.includes('DESCRIPTIONQTY') || upper.includes('PAGENO') || upper.includes('PAGE')
      ) {
        continue;
      }

      const cleanDescWithoutPack = trimmed.replace(/\\b\\d+[';]S\\b|\\b\\d+S\\b|\\b10S\\b|\\b15S\\b|\\b4S\\b|\\b14S\\b/gi, '').trim();
      const prod = matchMasterProduct(cleanDescWithoutPack) || matchMasterProduct(trimmed);

      // If report format is ITEM_WISE: Headers are Products!
      if (reportFormat === 'ITEM_WISE') {
        if (prod) {
          currentProduct = prod;
        }
      } else {
        // If report format is PARTY_WISE: Headers are Chemists!
        if (!prod && /[A-Z]/i.test(trimmed) && trimmed.length >= 3) {
    ...
    // =========================================================================
    if (reportFormat === 'ITEM_WISE') {
      // In ITEM_WISE, dataRow.desc is the CHEMIST NAME!
      if (currentProduct) {
      ...
      // In PARTY_WISE, dataRow.desc is the PRODUCT NAME!
...
print("✅ nagdaRetailerParser.ts updated with Dual-Invariant Auto-Detect Engine.")
...
print("\\n🎉 SUCCESS! Advanced Nagda Item-Wise & Cumulative Engine is Live!")
