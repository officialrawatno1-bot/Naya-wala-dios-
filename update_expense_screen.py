...
import { 
  ...
  Search, Stethoscope, CheckCircle
} from 'lucide-react';
...
export const ExpenseWorkspace: React.FC<Props> = ({ onBack }) => {
  ...
  // 🌟 REQUIREMENT 3: SCREENSHOT EXACT PERFORMANCE METRICS
  ...
  // 🌟 REQUIREMENT 1: SMART OTHER EXPENSE STATE & MASTER DOCTOR SEARCH
  ...
  // 🌟 REQUIREMENT 1 & 2: OPEN SMART OTHER EXPENSES BUILDER
  ...
  // Add Non-Doctor Category (WCFYH, Rapido, Auto Fare)
  ...
  // Start Doctor Category selection (DOB, DOA, Fruits)
  ...
  const handleSelectDoctorForCategory = (doc: MslDoctor) => {
    ...
    let title = '';
    if (activeDoctorSearchCategory === 'DOB') title = `DOB Bill (${doc.doctorName})`;
    else if (activeDoctorSearchCategory === 'DOA') title = `DOA Bill (${doc.doctorName})`;
    else if (activeDoctorSearchCategory === 'FRUITS') title = `Fruits (${doc.doctorName})`;
  ...
  // 🌟 REQUIREMENT 2: APPLY SMART OTHER EXPENSE & REMARKS 100% RELIABLY
  const handleApplySmartExpense = () => {
    ...
    // Generate formatted breakdown remark string
    ...
    setStatusMsg(`🎉 Smart Other Expense (₹${sumOther}) applied & Remark updated to: "${breakdownText || 'None'}"!`);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] bg-emerald-950 text-emerald-300 border border-emerald-500/40 px-3 py-1 rounded-full font-mono font-bold flex items-center gap-1.5 shadow-lg shadow-emerald-950/50">
            <Sparkles size={13} className="text-emerald-400 animate-pulse" /> FREEZE PANE &bull; DOCTOR BILL SEARCH &bull; CBO METRICS
          </span>
        </div>
      </div>
      ...
      {/* 🌟 REQUIREMENT 3: SCREENSHOT EXACT PERFORMANCE METRICS TABLE */}
      {performanceMetrics && (
        <div className="bg-slate-900 p-5 rounded-2xl border border-purple-500/50 shadow-2xl space-y-4">
          ...
          {/* 🌟 EXACT LAVENDER NET EXPENSE CLAIMED BANNER FROM SCREENSHOT */}
      ...
      {/* 🌟 REQUIREMENT 1 & 2: SMART OTHER EXPENSE MODAL WITH MASTER DOCTOR SEARCH */}
      {activeSmartRowIdx !== null && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-3 md:p-4">
          <div className="bg-slate-900 border-2 border-yellow-500/60 rounded-3xl max-w-xl w-full p-5 shadow-2xl space-y-4 max-h-[92vh] flex flex-col">
            ...
            {/* 🌟 REQUIREMENT 1: NEW PRESET OPTIONS */}
            <div className="space-y-2">
              <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                Select Expense Category (Amounts will NOT be pre-filled):
              </span>
            ...
            {/* Added Items List with Manual Amount Input */}
            <div className="flex-1 overflow-y-auto space-y-2 border border-slate-800 rounded-2xl p-2.5 bg-slate-950/70 max-h-[220px]">
              <div className="text-[10px] text-slate-400 uppercase font-bold px-1">Selected Expense Items (Enter Amount):</div>
              
              {smartItems.length === 0 ? (
                <div className="py-6 text-center text-xs text-slate-500 font-mono">
                  No items selected yet. Tap any button above to add.
                </div>
              ) : (
                smartItems.map(it => (
                  <div key={it.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-900 border border-slate-800 gap-2 text-xs">
            ...
            {/* Modal Footer: Total & Apply to Row Remark */}
            <div className="pt-3 border-t border-slate-800 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  disabled={smartItems.length === 0}
                  className="px-4 py-2 bg-gradient-to-r from-yellow-500 to-amber-500 hover:from-yellow-400 disabled:opacity-50 text-slate-950 font-bold text-xs rounded-xl shadow-lg cursor-pointer flex items-center gap-1.5"
                >
                  <Check size={15} /> Apply &amp; Update Remark
                </button>
              </div>
            </div>
