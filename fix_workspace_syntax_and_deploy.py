...
export const ExpenseWorkspace: React.FC<Props> = ({ onBack }) => {
  ...
  // Open Smart Other Expenses Modal with saved breakdown memory
  ...
  // Safe In-App PDF Preview Handler (Safari Safe!)
  const handleExportPDF = () => {
    try {
      ...
      setStatusMsg("🎉 In-App PDF Preview Ready! Review in modal below.");
      setTimeout(() => setStatusMsg(null), 3500);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] bg-emerald-950 text-emerald-300 border border-emerald-500/40 px-3 py-1 rounded-full font-mono font-bold flex items-center gap-1.5 shadow-lg shadow-emerald-950/50">
            <Sparkles size={13} className="text-emerald-400 animate-pulse" /> FREEZE PANE &bull; IN-APP PDF &bull; EXACT CBO FORMAT
          </span>
        </div>
      </div>
      ...
      {/* 6. ALLOWANCE CALCULATION BOX & HEAD/TYPE/AMOUNT */}
      ...
      {performanceMetrics && (
        <div className="bg-slate-900 p-5 rounded-2xl border border-purple-500/50 shadow-2xl space-y-4">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 pb-2 border-b border-slate-800">
            ...
            {/* Print Mode: With Values / Blank Values */}
            <div className="flex items-center gap-2">
      ...
      {/* 9. IN-APP PDF PREVIEW MODAL (SAFARI-SAFE, NO REDIRECT TO HOME SCREEN) */}
      {pdfPreviewModal && (
