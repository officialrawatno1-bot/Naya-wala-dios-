...
echo "=========================================================================="
echo "🖥️ [STARTING LIVE TERMINAL WEB ENGINE (PORT 9000)]..."
echo "=========================================================================="

pkill -f "uvicorn live_terminal_server:app" 2>/dev/null || true
pkill -f "live_terminal_9000.py" 2>/dev/null || true
sleep 1

# Create unified log file
touch /tmp/terminal_stream.log
echo "[$(date +'%H:%M:%S')] 🚀 Live Terminal Web Engine Initialized." > /tmp/terminal_stream.log

# 1. Create Web Streamer Server
...
# 2. Launch background server on Port 9000
nohup python3 live_terminal_server.py > /dev/null 2>&1 &
sleep 2

# 3. Set Port 9000 to Public
CURRENT_CS="${CODESPACE_NAME:-local}"
gh codespace ports visibility 9000:public -c "$CURRENT_CS" 2>/dev/null || true

echo ""
echo "=========================================================================="
echo "🎉 LIVE TERMINAL IS ONLINE ON PORT 9000!"
echo "👉 IPAD WEB TERMINAL LINK: https://${CURRENT_CS}-9000.app.github.dev"
echo "=========================================================================="
echo "🚀 Now running CBO Expense Extractor and streaming live to Port 9000..."
echo "=========================================================================="
echo ""

# 4. Run CBO Expense Extraction and pipe to stream
python3 test_cbo_expense.py 2>&1 | tee -a /tmp/terminal_stream.log
