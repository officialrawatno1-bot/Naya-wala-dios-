...
const REAL_AUGUST_ROWS: ExpenseDayRow[] = [
...
export const ExpenseWorkspace: React.FC<Props> = ({ onBack }) => {
  ...
  // Blank values toggle for CBO Performance Summary
  const [blankPerfValues, setBlankPerfValues] = useState<boolean>(false);

  // Hide Misc values toggle
  ...
  // 🌟 CLEAN DIRECT PDF DOWNLOAD (NO SAFARI TAB OVERWRITE OR HOME JUMP!)
  const handleExportPDF = () => {
    try {
      ...
      // Direct clean file download
      res.doc.save(res.fileName);
      setStatusMsg(`🎉 PDF Downloaded: '${res.fileName}' saved directly to iPad!`);
      setTimeout(() => setStatusMsg(null), 3500);
    } catch (e: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-4 md:p-8 max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-800">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] bg-emerald-950 text-emerald-300 border border-emerald-500/40 px-3 py-1 rounded-full font-mono font-bold flex items-center gap-1.5 shadow-lg shadow-emerald-950/50">
            <Sparkles size={13} className="text-emerald-400 animate-pulse" /> FLUID TOUCH SCROLL &bull; CLEAN DIRECT PDF DOWNLOAD
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-xs md:text-sm mt-1">
            BE: {headerInfo.name} ({headerInfo.code}) &bull; HQ: {headerInfo.hq} &bull; Fluid Touch Scrolling &bull; Direct PDF Download
          </p>
        </div>
      ...
      {/* 🌟 19-COLUMN DAY-WISE EXPENSE TABLE (SMOOTH FLUID SCROLL ON IPAD - ZERO FREEZE) */}
      <div className="bg-slate-900 p-4 rounded-2xl border border-slate-800 shadow-xl space-y-3">
        ...
        <div className="overflow-x-auto max-h-[540px] border border-slate-800 rounded-xl relative shadow-xl">
          <table className="w-full text-left text-xs border-separate border-spacing-0">
            <thead className="sticky top-0 z-30 bg-slate-950">
              <tr>
                ...
                <th className="p-2.5 text-center w-16 text-slate-200">Payable KM</th>
                ...
                <th className="p-2.5 text-center w-28 text-yellow-300">Other Exp (⚡)</th>
                      ...
                      <td className="p-1">
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            ...
                            title="Open Smart Breakdown (WCFYH, Rapido, Auto, Doctor DOB/DOA, Fruits, Custom)"
                          >
                            <Calculator size={12} />
                          </button>
                        </div>
      ...
      {/* ALLOWANCE CALCULATION BOX & HEAD/TYPE/AMOUNT */}
      ...
      {activeSmartRowIdx !== null && (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md z-50 flex items-center justify-center p-3 md:p-4">
          <div className="bg-slate-900 border-2 border-yellow-500/60 rounded-3xl max-w-xl w-full p-5 shadow-2xl space-y-4 max-h-[92vh] flex flex-col">
            ...
            {/* SAVED ITEMS LIST */}
            <div className="flex-1 overflow-y-auto space-y-2 border border-slate-800 rounded-2xl p-2.5 bg-slate-950/70 max-h-[200px]">
