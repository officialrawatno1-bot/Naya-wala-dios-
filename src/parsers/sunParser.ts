...
export async function parseSunFile(file: File, partyName: string = 'Sun Distributors'): Promise<PartyParseSummary> {
  ...
  if (isPdf) {
    ...
    // Multi-tier Packing Regex (supports 1*1*14 TAB, 1*10, 1*4, 10S, etc.)
        ...
        if (rawName) {
          ...
          if (matched) {
            ...
            if (numTokens.length === 7) {
              ...
              sales = numTokens[numTokens.length - 4] || numTokens[0];
              closing = numTokens[numTokens.length - 3] || numTokens[1];
            }
